...
async function combineObjects(urls) {
  let mergeJobBoard = [];
  for (let url of urls) {
    const data = await downloadObject(url);
    data.departments.forEach((department) => {
      mergeJobBoard.push(department);
    });
  }
  return mergeJobBoard;
}
...
async function testCombine() {
  ...
  const mergeJobBoard = await combineObjects(objectUrls);
  var allJobBoards = [];
  ...
  select_query.forEach((value) => {
    value.addEventListener("change", (event) => {
      ...
      const matches = allJobBoards.filter((department) => {
        const dept_name =
          department.id.toString().toUpperCase().indexOf(getDepartments) >= 0;
